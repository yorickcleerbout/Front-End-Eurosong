<template>
    <div class="carousel">
        <img src="../assets/arrowleft.png" @click="changeIndex(-1)" width="100">

        <div v-for="(song, index) in items" :key="song.id" v-if="index == activeIndex">
            {{ song.artist.name }} - {{ song.title }}
            <iframe :src="song.spotify" width="800" height="80" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>
        </div>
        
        <img src="../assets/arrowright.png" @click="changeIndex(1)" width="100">

    </div>
</template>

<script>
export default {
    name: "Carousel",
    props: [
        "items",
        "activeIndex"
    ],
    methods: {
        changeIndex(value) {
            let index = this.activeIndex;

            index += value;
            console.log(this.items.length)
            if (index < 0) index = this.items.length -1;
            else if (index == this.items.length) index = 0;

            this.$emit('change-index', index);
        }
    }
}
</script>